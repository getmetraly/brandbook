      ...
      <DocsSection id="layout" title="Layout composition">
        <div className="dashboard-layout-demo">
          <aside className="sidebar-demo panel">
            <div className="logo" style={{ marginBottom: 28 }}><span className="logo-mark"><span className="metraly-pulse-marker" /></span><span>Metraly</span></div>
            {["Overview", "Boards", "Signals", "Incidents", "Settings"].map((item, index) => <div className={index === 1 ? "sidebar-item active" : "sidebar-item"} key={item}>{index === 1 ? <span className="metraly-pulse-marker" /> : null}{item}</div>)}
          </aside>
