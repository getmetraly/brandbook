...
// ─── Minimal inline SVG icon — scenario shell + right-rail only ───
// Picker card icons live inside WidgetPickerCard itself.
...
// ─── Widget body components (scenario-only inline data viz) ───────
    ...
    <div style={{ flex: 1, padding: 12, display: "flex", flexDirection: "column", justifyContent: "space-between", minHeight: 0 }}>
      ...
      {spark ? (
        <div style={{ display: "flex", alignItems: "end", gap: 3, height: 36 }}>
          {spark.slice(-12).map((point, i) => (
            <span key={i} style={{ width: 6, height: `${Math.max(8, Math.min(34, point / 1.1))}px`, background: i > 7 ? "var(--m-purple-500)" : "var(--m-cyan-500)", borderRadius: 999, opacity: 0.78 }} />
          ))}
        </div>
      ) : null}
    </div>
...
// Simplified SVG bar chart — scales to widget width via preserveAspectRatio="none".
function DeployFreqBody() {
  ...
  return (
    <div style={{ flex: 1, padding: "8px 12px 8px", display: "flex", flexDirection: "column", gap: 8, minHeight: 0 }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ flex: 1, minHeight: 0 }}
        preserveAspectRatio="none" aria-hidden="true">
        {bars.map((v, i) => {
          const bh = (v / max) * (H - 4);
          return <rect key={i} x={i * 14 + 1} y={H - bh} width={12} height={bh} rx={2}
            fill="var(--m-cyan-500)" opacity={0.65} />;
        })}
        {/* Rollback overlay dots */}
        {rollbacks.map((r, i) => r
          ? <circle key={i} cx={i * 14 + 7} cy={H - (bars[i] / max) * (H - 4) - 5} r={3}
              fill="var(--m-purple-500)" opacity={0.9} />
          : null
        )}
      </svg>
      <div style={{ display: "flex", gap: 14, fontFamily: "var(--m-font-mono)", fontSize: 10, color: "var(--m-fg-3)", flexShrink: 0 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
          <span style={{ width: 10, height: 8, background: "var(--m-cyan-500)", borderRadius: 2, opacity: 0.65 }} />
          deploys
        </span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
          <span style={{ width: 8, height: 8, borderRadius: 999, background: "var(--m-purple-500)", opacity: 0.9 }} />
          rollbacks
        </span>
      </div>
    </div>
      ...
      {cells.map((c, i) => (
        <div key={i} style={{ background: "var(--m-bg-1)", border: "1px solid var(--m-line-faint)", borderRadius: 6, padding: "8px 10px", display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
            <span style={{ fontFamily: "var(--m-font-mono)", fontSize: 17, color: "var(--m-fg-0)" }}>{c.value}</span>
            <span style={{ fontFamily: "var(--m-font-mono)", fontSize: 10, color: c.tone === "warn" ? "var(--m-warn)" : "var(--m-ok)" }}>{c.trend}</span>
          </div>
          <div style={{ fontFamily: "var(--m-font-mono)", fontSize: 9, color: "var(--m-cyan-500)" }}>· {c.tier}</div>
        </div>
      ))}
    </div>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        {segs.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, fontFamily: "var(--m-font-mono)", color: "var(--m-fg-2)" }}>
            <span style={{ width: 8, height: 8, background: s.color, borderRadius: 2, opacity: 0.7, flexShrink: 0 }} />
...
function BlockedWorkBody() {
  ...
  const sevColor = (sev: string) =>
    sev === "error" ? "var(--m-err)" : sev === "warn" ? "var(--m-warn)" : "var(--m-line)";
  return (
    <div style={{ flex: 1, overflow: "auto", padding: 8, display: "flex", flexDirection: "column", gap: 4 }}>
      {items.map((a, i) => (
        <div key={i} style={{
          ...
          borderRadius: 4,
          borderLeft: `2px solid ${sevColor(a.sev)}`,
        }}>
          <span style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
            <span style={{ fontSize: 11, color: "var(--m-fg-1)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {a.stage} · {a.title}
            </span>
...
function PRReviewTableBody() {
  const rows = [
    ...
    { team: "mobile",         open: 4,  first: 3.0,  merge: "14h", stale: 0 },
  ];
  const th: React.CSSProperties = {
    padding: "6px 10px", textAlign: "left",
    fontSize: 10, fontFamily: "var(--m-font-mono)",
    color: "var(--m-fg-3)", fontWeight: 600, letterSpacing: "0.03em", textTransform: "uppercase",
    borderBottom: "1px solid var(--m-line-faint)",
    background: "var(--m-bg-3)",
  };
  const td: React.CSSProperties = {
    padding: "6px 10px", textAlign: "left",
    fontSize: 11, fontFamily: "var(--m-font-mono)",
    color: "var(--m-fg-1)",
    borderBottom: "1px solid var(--m-line-faint)",
  };
  return (
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              ...
              <td style={{ ...td, textAlign: "right" }}>{r.open}</td>
              <td style={{ ...td, textAlign: "right", color: r.first > 8 ? "var(--m-warn)" : "var(--m-fg-1)" }}>{r.first}h</td>
              <td style={{ ...td, textAlign: "right" }}>{r.merge}</td>
              <td style={{ ...td, textAlign: "right", color: r.stale > 0 ? "var(--m-warn)" : "var(--m-fg-2)" }}>{r.stale}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const widgets: WidgetDef[] = [
  // Row 1 — DORA scalars
  { id: "deploy-freq",   title: "Deployment frequency",    kind: "dora/deploy-freq",  body: "metric",
    value: "24 / day", delta: "▲ 18% vs −14d",    deltaTone: "ok",
    spark: [4,6,5,7,8,6,9,11,10,12,14,13,15,14,12,16,18,17,15,19,21,18,22,24],
    position: { col: 1,  row: 1, w: 3, h: 1 } },
  { id: "lead-time",     title: "Lead time for changes",   kind: "dora/lead-time",    body: "metric",
    value: "41h",      delta: "▼ 6h vs −14d",     deltaTone: "ok",
    spark: [62,60,58,56,55,54,52,50,49,48,47,46,45,44,44,43,43,42,42,42,41,41,41,41],
    selected: true,
    position: { col: 4,  row: 1, w: 3, h: 1 } },
  { id: "cfr",           title: "Change failure rate",     kind: "dora/cfr",          body: "metric",
    value: "4.2%",     delta: "▲ 0.8% vs −14d",   deltaTone: "warn",
    spark: [3.0,3.1,3.0,2.9,3.2,3.4,3.3,3.5,3.6,3.5,3.7,3.8,3.9,3.8,4.0,3.9,4.1,4.0,4.2,4.1,4.3,4.2,4.2,4.2],
    dragging: true,
    position: { col: 7,  row: 1, w: 3, h: 1 } },
  { id: "ci-status",     title: "CI failure rate",         kind: "ci/fail",           body: "error",
    state: "error",
    position: { col: 10, row: 1, w: 3, h: 1 } },

  // Rows 2-3 — deploy chart (wide) + DORA overview
  { id: "deploy-chart",  title: "Deploys / day · last 14d", kind: "dora/deploy-freq", body: "deployFreq",
    position: { col: 1, row: 2, w: 8, h: 2 } },
  { id: "dora-overview", title: "DORA overview",           kind: "dora/overview",     body: "doraOverview",
    state: "new",
    position: { col: 9, row: 2, w: 4, h: 2 } },

  // Rows 4-5 — cycle time (tall, left) + 4 metric widgets (right, 2 rows)
  { id: "cycle-time",    title: "Cycle time breakdown",    kind: "flow/cycle",        body: "cycleTime",
    position: { col: 1, row: 4, w: 4, h: 2 } },
  { id: "review-lat",    title: "PR review latency · p50", kind: "review/latency",   body: "metric",
    value: "5.4h",     delta: "▼ 1.1h vs −14d",   deltaTone: "ok",
    spark: [9,8.5,8,7.5,7.2,7,6.8,6.6,6.5,6.3,6.2,6,5.9,5.8,5.7,5.6,5.5,5.5,5.4,5.4,5.4,5.4,5.4,5.4],
    position: { col: 5, row: 4, w: 4, h: 1 } },
  { id: "flaky",         title: "Flaky builds · 7d",       kind: "ci/flaky",          body: "metric",
    value: "37",       delta: "▲ 9 vs −7d",        deltaTone: "warn",
    spark: [12,14,16,15,18,20,22,24,25,28,30,32,33,34,35,36,37,37,37,37,37,37,37,37],
    position: { col: 9, row: 4, w: 4, h: 1 } },
  { id: "blocked",       title: "Blocked work",            kind: "flow/blocked",      body: "blocked",
    position: { col: 5, row: 5, w: 4, h: 1 } },
  { id: "team-health",   title: "Team delivery health",    kind: "team/health",       body: "metric",
    value: "78 / 100", delta: "▲ 4 vs −14d",      deltaTone: "ok",
    spark: [70,71,71,72,72,73,73,74,74,74,75,75,76,76,76,77,77,77,77,78,78,78,78,78],
    position: { col: 9, row: 5, w: 4, h: 1 } },

  // Rows 6-7 — full-width PR review table
  { id: "review-table",  title: "PR review latency by team", kind: "review/by-team", body: "prTable",
    position: { col: 1, row: 6, w: 12, h: 2 } },
];

function widgetBody(w: WidgetDef): React.ReactNode {
  if (w.body === "metric") {
    return (
      <MetricBody
        value={w.value ?? "—"}
        delta={w.delta ?? ""}
        deltaTone={w.deltaTone}
        spark={w.spark}
      />
    );
  }
  ...
  if (w.body === "deployFreq")   return <DeployFreqBody />;
  if (w.body === "doraOverview") return <DoraOverviewBody />;
  if (w.body === "cycleTime")    return <CycleTimeBarsBody />;
  if (w.body === "blocked")      return <BlockedWorkBody />;
  if (w.body === "prTable")      return <PRReviewTableBody />;
  return null;
}
...
function DashboardEditorScenario() {
  return (
    // 3-column app chrome: sidebar | scrollable canvas | right rail
    <div style={{
      display: "flex",
      width: "100%",
      height: 900,
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: "var(--m-bg-2)", border: "1px solid var(--m-cyan-500)",
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {sidebarNav.map((item) => (
            <div key={item.id} style={{
              ...
              fontSize: "var(--m-fs-12)", cursor: "pointer",
            }}>
              {item.active && (
                <span style={{ position: "absolute", left: 0, top: 6, bottom: 6, width: 2, background: "var(--m-cyan-500)", borderRadius: 2 }} />
              )}
              <Icon name={item.icon} size={13} />
              <span style={{ flex: 1 }}>{item.label}</span>
              {item.badge != null && (
                <span style={{ fontFamily: "var(--m-font-mono)", fontSize: 9, color: "var(--m-cyan-500)", background: "var(--m-cyan-bg)", padding: "1px 5px", borderRadius: 999, border: "1px solid var(--m-cyan-500)" }}>
                  {item.badge}
                </span>
              )}
            </div>
      ...
      {/* ── Center: topbar + scrollable main ────────────────────── */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>

        {/* Topbar — no rounded card, border-bottom */}
        <header style={{
        ...
        <main style={{ flex: 1, overflow: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12, minWidth: 0 }}>
          ...
          {/* Edit mode banner — dashed border, plain mono text, no decorative elements */}
          <div style={{
            display: "flex", alignItems: "center", gap: 10,
            padding: "8px 12px",
            ...
            <span style={{ color: "var(--m-fg-2)", textTransform: "none", letterSpacing: 0 }}>
              · Drag widgets with grip dots · Resize from corner handles · Drag from library to add
            </span>
            <span style={{ flex: 1 }} />
          ...
          {/* 12-column dashboard grid — rows auto-sized, min 150px */}
          <div style={{
            display: "grid",
            ...
            {widgets.map((w) => (
              <div key={w.id} style={{
                gridColumn: `${w.position.col} / span ${w.position.w}`,
                gridRow: `${w.position.row} / span ${w.position.h}`,
              }}>
                <DashboardWidget
                  id={w.id}
                  title={w.title}
            ...
            {/* Drop zone — idle, pulse-free (DashboardDropZone carries data-pulse="off") */}
            <div style={{ gridColumn: "1 / span 12" }}>
              <DashboardDropZone state="idle" label="Drag a widget here · or click + Add widget" />
            </div>

            {/* Resize handle specimens */}
      ...
      {/* ── Right rail — Widget library, 320px, no rounded card ──── */}
      <aside style={{
        width: 320,
        flexShrink: 0,
        background: "var(--m-bg-1)",
        ...
        <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--m-line-faint)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--m-bg-2)", border: "1px solid var(--m-line)", borderRadius: "var(--m-r-2)", padding: "0 10px", height: "var(--m-control-h)" }}>
            ...
            <input
              ...
              readOnly
              style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "var(--m-fg-1)", fontFamily: "var(--m-font-ui)", fontSize: "var(--m-fs-12)" }}
            />
          </div>
        </div>

        {/* Scrollable widget list — all cards pulse-free */}
