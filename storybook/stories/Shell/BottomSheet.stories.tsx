  ...
  return (
    <ThemeProvider>
      <div style={{ minHeight: 640, background: "var(--m-bg-0)", padding: 24 }}>
        <MetralyButton
          variant="primary"
          iconLeft={<MetralyIcon name="boxes" size="sm" />}
...
function FocusVerificationPreview() {
  const [open, setOpen] = React.useState(false);
    ...
    <ThemeProvider>
      <div style={{ minHeight: 960, background: "var(--m-bg-0)", padding: 24, display: "grid", gap: 12 }}>
        ...
        <MetralyBottomSheet
          ...
          onOpenChange={setOpen}
          title="Widget library"
          description="Sheet should restore focus and keep scroll inside the sheet body."
          maxHeight="72dvh"
        >
...
export const Default: Story = {
  name: "Widget library (mobile trigger)",
  parameters: { viewport: { defaultViewport: "mobile2" } },
  render: () => <BottomSheetPreview />,
};

export const FocusTrap: Story = {
  name: "Focus trap / scroll lock verification",
  parameters: { viewport: { defaultViewport: "mobile1" } },
  render: () => <FocusVerificationPreview />,
};
